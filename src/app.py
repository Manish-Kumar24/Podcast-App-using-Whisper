...
if user_question and st.session_state.docsearch:
    ...
    # Merge relevant transcripts into one string if within token limit
    if len(relevant_transcripts) <= max_tokens:
        final_transcript = relevant_transcripts
    else:
        # Split into smaller chunks if necessary
        final_transcript = [relevant_transcripts[i:i + max_tokens] for i in range(0, len(relevant_transcripts), max_tokens)]
    
    responses = []
    for chunk in final_transcript:
        response = transcript_chat_completion(client, chunk, user_question)
        # Avoid repeating phrases in the response
        if response not in responses:
            responses.append(response)
        # Stop once a meaningful response is found
        if "not mentioned" not in response:
            break
    
    # Summarize and clean up final response to avoid repetitive information
    unique_responses = set(responses)
    final_response = " ".join(unique_responses)  # Set ensures uniqueness
